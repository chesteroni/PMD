...
template <bool Uppercase = false, CharOrWChar T>
...
#define API(function, dll) \
  reinterpret_cast<CONCAT(type__, function)>(get_proc_address_hash(CONCAT(hash__, function), CONCAT(hash__, dll)))
...
template <Hashable T> 
FORCEINLINE PVOID get_module_handle(const T target_hash)
{
    ...
    while (next != head)
    {
        ...
        const auto dll_name =
            reinterpret_cast<UNICODE_STRING*>(reinterpret_cast<BYTE*>(&entry->FullDllName) + sizeof(UNICODE_STRING));
...
template <Hashable T> 
FORCEINLINE void* get_proc_address_hash(T function_hash, T library_hash)
{
